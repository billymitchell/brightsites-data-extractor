...
function resolveTokenKey(token) {
  if (token === undefined || token === null) return 'default';
  const out = String(token).trim();
  return out || 'default';
}

function noteUsage(tokenKey) {
  if (tokenKey && tokenKey !== 'default') usage.increment(tokenKey, 1);
}
...
async function safeFetch(url, opts = {}, retries = 4, delay = 1000) {
  const { isCanceled, tokenKey: rawTokenKey, headers: inputHeaders, ...fetchOpts } = opts;
  const tokenKey = resolveTokenKey(rawTokenKey);
  const headers = Object.assign({}, inputHeaders || {});
  if (tokenKey !== 'default') headers['X-Application-Token'] = tokenKey;
  for (let i = 0; i <= retries; i++) {
    try {
      throwIfCanceled(isCanceled, 'Report job canceled before request dispatch.');
      await waitForRequestSlot(tokenKey);
      throwIfCanceled(isCanceled, 'Report job canceled while waiting for the next request slot.');
...
async function fetchAllPages(path, params = {}, pageSize = 200, opts = {}) {
  const { onPage, isCanceled } = opts;
  const out = [];
  let page = 1;
  while (true) {
    throwIfCanceled(isCanceled, 'Report job canceled while fetching paginated data.');
    ...
    const p = Object.assign({}, params, { page, per_page: pageSize });
    const qs = new URLSearchParams();
    Object.entries(p).forEach(([k, v]) => {
      if (v !== undefined && v !== null && v !== '') qs.append(k, String(v));
    });
    const url = `${base}${path}?${qs.toString()}`;
    const res = await safeFetch(url, { method: 'GET', isCanceled, tokenKey: token });
    if (!res) break;
...
async function loadOrder(orderId, opts = {}) {
  ...
  try {
    ...
    const res = await safeFetch(url, { method: 'GET', isCanceled: opts.isCanceled, tokenKey: token });
    if (!res) return {};
    const json = await res.json();
